...
{
	real_t coefficient = -945.0 / (32.0*PI*pow(h_constant, 9));
	real_t hSquared = h_constant * h_constant;
...
{
	real_t coefficient = -45.0 / (PI*pow(h_constant, 6));

	real_t radius = sqrt(radiusSquared);

	return (coefficient * pow(h_constant - radius, 2)) * diffPosition / radius;
}
...
real_t WviscosityLaplacian(real_t radiusSquared)
{
	...
	return coefficient * (h_constant - radius);
}

real_t Wpoly6(real_t radiusSquared)
{
	/*static*/ real_t coefficient = 315.0 / (64.0*PI*pow(h_constant, 9));
	/*static*/ real_t hSquared = h_constant * h_constant;
